...
function initQueryBank(){		
	...
	pageQueryDialogObj = new PageQueryDialog(
		true,'bankWindowDiv',
		600,300,
		basepath+'/nxpsappconf/nxpsBankinfo_queryCpimBank',
		['bankno','bankstatus','bankname','banktype'],
		[
			{header:'序号'},
			{header:'选框'},
            {header:'支付系统行号',dataIndex:'bankno',width:100,locked: true},
            {header:'行名全称',dataIndex:'bankname',width:160,locked: true},
            {header:'行号状态',dataIndex:'bankstatus',width:100,renderer:function(value){
                return getDictValue(value,bankstatusStore,'dictValue','dictValueDesc');
            }},
            {header:'行号类别',dataIndex:'banktype',width:100,renderer:function(value){
                return getDictValue(value,bankNoType,'dictValue','dictValueDesc');
            }}
