...
var author_param;
var service_name;
function requestAjax(url, submitData, callbackfunc,maskflag){
 ...
 function creatAuthWin(callbackfunc,jsonObj){
 	if(authorwindow==null){
		 authorwindow = new SelfFormWindow('authorwindow', '授权窗口', 200, 200, 100, 1,
				...
				{colIndex:0, field:{xtype : 'textfield', name:'passwd', 	  fieldLabel:'密码',   allowBlank:false,blankText:'密码'}}
				],
				[
					{text:'提交', formBind: true, handler : function(){
						...
						submitData,function(sRet){
							if(sRet.message&&sRet.message!=""){
								...
								authorwindow.close();
								requestAjax(service_name+author_url,author_param,function(sRet1){
										callbackfunc(sRet1);
								});
					}
				});
