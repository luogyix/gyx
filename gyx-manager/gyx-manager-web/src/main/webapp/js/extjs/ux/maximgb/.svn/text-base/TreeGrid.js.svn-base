...
/*
 * 使用事例：var mixedCollection=store.queryExact("_parent",record.get("_id"));
 * grid中有时候会统计所有子节点的值进行汇总时，store中只有query（）方法，但有时会有问题，出现统计子节点出现计算错误：
 * 这个时候就要重写store中原的的方法query了：如下
 */
...
Ext.ux.maximgb.tg.AdjacencyListStore = Ext.extend(Ext.ux.maximgb.tg.AbstractTreeStore,
{
    ...
    getRootNodes : function()
    {
        ...
        for (i = 0, len = records.length; i < len; i++) {
            if (Ext.isEmpty(records[i].get(this.parent_id_field_name))) {
    ...
    getRootNode : function(rc){
    	var root, store = this;
    	function _getNodeParent(record){
    		if(Ext.isEmpty(record.get(store.parent_id_field_name))){
    			root = record;
    			return;
    		}
    		var parent = store.getById(record.get(store.parent_id_field_name));
    		if(Ext.isEmpty(parent.get(store.parent_id_field_name))){
    			root = parent;
    		}else{
    			_getNodeParent(parent);
    		}
		}
    	_getNodeParent(rc);
    	return root;
    ...
    getNodeAllChildren : function(rc)
    {
    	var result = [], store = this;

    	_getNodeChildren(rc.id);
		function _getNodeChildren(_id){
			store.each(function(record){
				if(_id == record.get(store.parent_id_field_name)){
					result.push(record);
					if(!record.get(store.leaf_field_name))
						_getNodeChildren(record.id);//遍历选择
				}
			});
		}
        return result;
...
{   
    ...
    expandRow : function(record, skip_process)
    {
        ...
        record.hidden=Ext.isEmpty(record.hidden)?false:record.hidden;
    	
        row = this.getRow(index);
        row.hidden=record.hidden;//显示隐藏行
        row.style.display = record.hidden?'none':'block';//显示隐藏行
    ...
    onAdd : function(ds, records, index)
    {
        ...
        if (this.mainWrap) {
           //this.updateAllColumnWidths();
           this.processRows(0);
           this.grid.view.refresh(); //解决节点展开后位置左移BUG
        }
    },
...
//            this.updateUI();
    ...
    doLoad : function(start){
    	this.ownerCt.getSelectionModel().clearSelections();
        var o = {}, pn = this.getParams();
        o[pn.start] = start;
        o[pn.limit] = this.pageSize;
        for(condition in this.conditionParams ){
        	o[condition] = this.conditionParams[condition];
        }
...
Ext.reg('Ext.ux.maximgb.tg.NestedSetStore', Ext.ux.maximgb.tg.NestedSetStore);
