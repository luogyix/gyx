...
import com.agree.abt.model.dataAnalysis.TellerEfficiency;
...
public class TellerEfficiencyAction extends BaseAction {
	...
	public String exportExcel()throws Exception{
		...
		String path="TellerEfficiencyBook.xls";		
		String file = "柜员办理效率统计";
	...
	public void natp(JSONObject jsonObj,List list,Page pageInfo,User user,boolean blan) throws Exception{
		...
		cona.setBMSHeader("ibp.bms.b302.01", user);
...
//						/ pageInfo.getLimit() + 1;// 得到总页数
			...
			String loopNum = map.get("bttellerefficiencynum").get(0);
			int num = Integer.parseInt(loopNum);
			for (int i = 0; i < num; i++) {
				TellerEfficiency tellereff = new TellerEfficiency();
				//正常使用时将一下注释的打开
				tellereff.setBranch(map.get("branch").get(i).toString());
				tellereff.setBranch_name(map.get("branch_name").get(i).toString());
				tellereff.setDate(map.get("date").get(i).toString());
