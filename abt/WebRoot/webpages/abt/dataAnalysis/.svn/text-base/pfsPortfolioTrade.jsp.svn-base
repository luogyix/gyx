 ...
 */	    var systemUnits=pagereturn.field1;
		function loadPage(){
			...
			var conditionPanel = new SelfToolBarPanel('queryConditionPanel', '填单业务量统计查询', 120, 1,
				[
				...
				{rowIndex:0, field:{xtype:'textfield', 	name:'devicenum',fieldLabel:'机具编号',value:'', width:100}},
				{rowIndex:0, field:{xtype:'textfield', 	name:'trade_name_ch',fieldLabel:'业务名称',value:'', width:100}},
				{rowIndex:0, field:{xtype:'datefield',editable:false, 	name:'stratdate',   format:'Y-m-d',value:new Date(),allowBlank : false, fieldLabel:'起始日期'}},
			...
			var pagequeryObj = new PageQuery(
				...
				'<%=basePath%>/dataAnalysis/pfsPortfolioTrade_queryPfsPortfolioTrade',
				[
					'pfs_id','work_date','branch','devicenum','trade_id','trade_name_ch','pfscount','averagetime'
				],[
					{header:'全选'},
					{header:'复选框'},
					{header:'序号',dataIndex:'pfs_id',width:100},
					{header:'日期',dataIndex:'work_date',width:100},
					{header:'机构号',dataIndex:'branch',width:100},
					{header:'机具编号',dataIndex:'devicenum',width:150},
					{header:'业务编号',dataIndex:'trade_id',width:60},
					{header:'业务名称',dataIndex:'trade_name_ch',width:100},
					{header:'填单笔数',dataIndex:'pfscount',width:100},
					{header:'平均填单时间(分钟)',dataIndex:'averagetime',width:150}
				],
			...
			function onButtonClicked(btn_index){
				switch(btn_index){
					case 0:
						...
						var startdate=query_obj["stratdate"];
						var endate=query_obj["enddate"];
						var date=query_obj["date"];
  ...
  <body scroll="no">
	...
	<s:form action="/dataAnalysis/pfsPortfolioTrade_exportPfsPortfolioTrade" target="REPORTRESULTFRAME">
