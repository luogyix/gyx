...
//	var hrefTarget = conf.hrefTarget;
	...
	for (var i = 0; i < this.jsonArrayTree.length; i = i + 1) {
		...
		if(img!=undefined){
			var nodeconfig = {type:currentNode[this.jsonMeta.nodeType],id:currentNode[this.jsonMeta.nodeId].toString(), text:currentNode[this.jsonMeta.nodeName],etext:currentNode[this.jsonMeta.nodeNameEn], leaf:false, iconCls:(img), cls:(this.cls === undefined?'':this.cls[currentNode[this.jsonMeta.nodeType]])};
		}else{
				...
				if(nodeconfig.url !== undefined){	
					...
					node.on('click',function(n){
	    			    ...
	    			    if(tab){ 
	    			    	...
	    			    	var	htmlurl = '<iframe name="iftab_'+n.attributes.id+'" scrolling="auto" frameborder="0" width="100%" height="100%" src="'+n.attributes.url +'"></iframe>';
	    			    	var tab = c.add({
	    			    	...
	    			    	setTimeout("Ext.get('center-panel').unmask();",1000*10);
	    			    }
