...
PageQuery = function(whetherpage, divid, width, height, url, fields, colM, pageToolText, columnSortFunc,tbar,plugins){
	...
	this.pagingGrid.on('sortchange',function(field,sort){
		...
		if(this.submit_obj !== undefined){
			...
			this.selfPagingTool.doLoad(1,0);
		}
	},this);
...
PageQuery.prototype.queryPageByNumber = function(query_obj,num) {
	this.submit_obj = query_obj;
	this.selfPagingTool.resetPageInfo();
	this.selfPagingTool.doLoad(num,0);
}
/***********end************/
...
SelfPagingToolbar = Ext.extend(Ext.Toolbar, {
    ...
    lastText : "最后一页",
    refreshText : "Refresh",
    paramNames : {start: 'start', limit: 'limit', total: 'total',page:'page', resultList:'field1', resultObject:'field2',pageflag:'pageflag'},
...
/*****************************************************************************************/        
        ...
        this.pageCombo.on('select',this.onComboSelect,this);
/*****************************************************************************************/        
        if(!this.whetherpage){
        ...
        this.jsonData[this.paramNames.pageflag] = 4;
    ...
    onComboSelect : function(combo,record,index){
    	this.jsonData[this.paramNames.limit] = record.data.value;
    	if(this.parentComp.submit_obj!=undefined){
    		this.doLoad(1,0);
    	}
    },
    ...
    doLoad : function(start,pageflag){
		...
		this.parentComp.submit_obj[this.paramNames.pageflag] = this.jsonData[this.paramNames.pageflag];
    ...
    changePage: function(page){
        this.doLoad( ((page-1) * this.pageSize).constrain(0, this.store.getTotalCount()) ,4);
    ...
    onClick : function(which,num){
        ...
        switch(which){
            ...
            break;
            case "prev":
                this.doLoad(this.jsonData[this.paramNames.start] - 1,2);
            break;
            case "next":
                this.doLoad(this.jsonData[this.paramNames.start] + 1,3);
            break;
            case "last":
                this.doLoad(-1,1);
            break;
